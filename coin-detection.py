...
def get_histogram(image):
    ...
    cv2.circle(mask, (width, height), radius=60, color=(255, 255, 255))

    # calcHist expects a list of images, color channels, mask, bins, ranges
    height = cv2.calcHist([image], [0, 1, 2], mask, [16, 16, 16], [0, 256, 0, 256, 0, 256])
...
if circles is not None:
    ...
    for (x, y, d) in circles:
        ...
        cv2.circle(output, (x, y), d, (187, 255, 0), 3)
        cv2.putText(output, material, (x - 40, y), cv2.FONT_HERSHEY_PLAIN, 1.5, (187, 255, 0), thickness=2, lineType=cv2.LINE_4)
...
while i < len(diameter):
    ...
    cv2.putText(output, t, (x - 40, y + 22), cv2.FONT_HERSHEY_PLAIN, 1.5, (3, 186, 252), thickness=2, lineType=cv2.LINE_4)
    i += 1
